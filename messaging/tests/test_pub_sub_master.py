...
from cereal.messaging.tests.test_messaging import events, random_sock, random_socks, \
                                                  random_bytes, random_controls_state, assert_controls_state, \
                                                  zmq_sleep
...
class TestSubMaster(unittest.TestCase):
  ...
  def test_getitem(self):
    ...
    sm.update(1000)
    assert_controls_state(msg.controlsState, sm[sock])
  ...
  def test_conflate(self):
    ...
    for i in range(n+1):
      msg = messaging.new_message(sock)
      msg.controlsState.startMonoTime = i
      pub_sock.send(msg.to_bytes())
      time.sleep(0.01)
    sm.update(1000)
    self.assertEqual(sm[sock].startMonoTime, n)
